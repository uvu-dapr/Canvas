...
| Images on any Canvas page | XVIII, XIX (GitHub), XXI |
...
### 1.9 Image Embedding Methods

**Option 1 - Inline SVG (preferred for diagrams):** Paste raw `<svg>...</svg>` markup directly into the Canvas HTML editor. No file upload, no external URL. The graphic is part of the page and cannot break.

Accessibility requirement: Add `role="img"` and `aria-label="description"` to the opening `<svg>` tag.

**Option 2 - GitHub raw URL (required for screenshots and photos):** Point an `<img src="https://raw.githubusercontent.com/...">` tag at the GitHub repository. See Parts XVIII and XXI for the full GitHub image workflow.

**Option 3 - Wikimedia Commons hotlink (for CC-licensed photos):** Use `<figure>/<figcaption>` with full CC BY-SA attribution. See template below.

**Avoid:** Base64 data URIs (HTML becomes enormous and Canvas may truncate), Canvas Files uploads (more steps, breaks copy-paste workflow).
...
<div style="font-family:Arial, Helvetica, sans-serif; max-width:900px;
  ...
  <div style="background-color:#E3F2FD; border-left:4px solid #0D47A1;
              padding:12px 16px; margin:16px 0; display:flex;
              align-items:flex-start; gap:12px; border-radius:0 4px 4px 0;"
       role="note" aria-label="Note">
...
Every Canvas page that uses a callout box must use one of the five approved types. Custom colors and custom icon shapes are not permitted.

### 20.1 Icon Specification

All icons are inline SVG at `viewBox="0 0 24 24"` with fixed attributes: `fill="none"`, `stroke-width="2.2"`, `stroke-linecap="round"`, `stroke-linejoin="round"`. Include `role="img" aria-hidden="true"` on every icon. Size: `width="22" height="22"` inside callout boxes.

### 20.2 Five Callout Box Types -- Locked Colors

| Type | Icon | Border / Stroke | Background Fill | When to Use |
|---|---|---|---|---|
| Required | Green shield with checkmark | #1B5E20 | #E8F5E9 | Mandatory rules, program standards, must-follow instructions |
| Note | Blue info circle | #0D47A1 | #E3F2FD | Clarifications, reminders, helpful context |
| Warning | Yellow yield triangle | #F57F17 | #FFF8E1 | Common mistakes, known gotchas, proceed with caution |
| Critical / Stop | Red x-circle | #C62828 | #FBE9E7 | Hard rules, enforcement policies, absolute prohibitions |
| Tip / Pro tip | Purple question circle | #4A148C | #F3E5F5 | Helpful suggestions, shortcuts, efficiency notes |

### 20.3 Callout Box HTML Template

```html
<div style="background-color:#E8F5E9; border-left:4px solid #1B5E20; padding:12px 16px;
     margin:16px 0; display:flex; align-items:flex-start; gap:12px;
     border-radius:0 4px 4px 0;" role="note" aria-label="[description]">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="22" height="22"
       style="flex-shrink:0; margin-top:2px;" fill="none" stroke="#1B5E20"
...
**SVG path data by type:**

Required (shield with checkmark):
```
...
Warning (yield triangle):
```
<path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
```

Critical / Stop (x-circle):
```
<circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>
```

Tip / Pro tip (question circle):
```
