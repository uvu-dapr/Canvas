...
- **Outer wrapper MUST pin `background-color:#ffffff`.** The Canvas iOS app aggressively applies dark mode to any element without an explicit background, leaving dark `#212121` text rendered on a near-black background — effectively invisible. The macOS browser does not do this, so the bug only surfaces on mobile and is easy to miss during desktop QA. Pin white on the outermost wrapper div, and add `padding:0 12px` while you're there to keep body text from running flush against narrow mobile screen edges. There is no Canvas-supported way to make pages auto-invert (no `<style>` tags, no JS, no `prefers-color-scheme`) — pinning white is the program standard. Students who want dark mode can use the in-app "Switch To Light Mode" / "Switch To Dark Mode" toggle, which only affects Canvas chrome, not page content.
- **Every `<div>` with a dark background MUST set its own `color:#ffffff` explicitly.** When Canvas's RCE saves a page, it runs CSS through TinyMCE's normalizer. If a div has a dark background but no explicit text color, the normalizer "helpfully" injects one — and has been observed injecting `color:#ffffffff` (8-digit hex, alpha channel). 8-digit hex is technically valid CSS Color Level 4 but is flagged by older Panorama scans and read inconsistently by assistive tech. To prevent the editor from injecting anything, always set `color:#ffffff` yourself on any div whose visible text relies on inheritance over a dark background. Examples: the green What to Submit banner, any colored title bar, any callout with a saturated background.
- **Lowercase all hex colors in source HTML.** TinyMCE lowercases on save anyway. Writing `#1b5e20` instead of `#1B5E20` keeps source and stored copy identical so diffs stay clean.
- **Don't use shorthand `border:none; border-top:...` on `<hr>` elements.** TinyMCE always expands `<hr>` border shorthand to longhand on save (`border-width`, `border-style`, `border-color`, `border-image` separately) AND injects `color:#000000; background-color:#ffffff;` whether you wanted them or not. Visual result is identical, but the stored HTML balloons. Either accept the bloat or write longhand from the start: `border-style:none; border-top-style:solid; border-top-width:3px; border-top-color:#1b5e20;`.
- **Never stack two `<hr>` elements as a visual divider.** A `<hr>` is a thematic break, semantically. Two stacked `<hr>` tags tell a screen reader "two thematic breaks here," which is wrong when the goal is one divider that happens to look like a double line. Use a single `<hr>` plus padding, or a styled `<div>` with top and bottom borders if you really need the double-line look.
...
### Outer wrapper requirements (mobile dark mode fix)

The outer wrapper carries five required style declarations and one required for mobile:

- `font-family:Arial, Helvetica, sans-serif` - program font
- `max-width:900px; margin:0 auto` - desktop centering and reading-width cap
- `color:#212121` - body text color
- `line-height:1.6` - body line height
- `background-color:#ffffff` - **pins the page background to white. Without this, the Canvas iOS app applies dark mode to the wrapper and renders `#212121` text on a dark background, making the entire page nearly unreadable on mobile. The macOS browser ignores iOS dark mode so this bug only surfaces on phone, which means it gets missed during desktop QA. Always pin white.**
- `padding:0 12px` - keeps body text off the screen edges on narrow mobile viewports

There is no Canvas-supported way to make pages auto-invert for dark mode (no `<style>` tags, no JS, no `prefers-color-scheme` media queries). Pinning white is the program standard.
...
- **Outer wrapper missing `background-color:#ffffff` → page renders unreadable in the Canvas iOS app's dark mode (dark text on dark background). The macOS browser hides this bug, so it only surfaces during mobile QA. Every wrapper needs `background-color:#ffffff; padding:0 12px;` in addition to the standard font/width/color declarations. See §6 outer wrapper requirements.**
- **Div with a dark background but no explicit `color` → TinyMCE auto-injects `color:#ffffffff` (8-digit hex, alpha) on save. Set `color:#ffffff` yourself on every dark-background div. The green What to Submit banner is the most common offender.**
- **`<hr style="border:none; border-top:...">` shorthand → TinyMCE expands to longhand on save and adds `color:#000000; background-color:#ffffff;` you didn't ask for. Visual result unchanged but HTML balloons. Either accept the rewrite or write longhand from the start.**
- **Two stacked `<hr>` elements as a "double line" divider → screen readers announce two thematic breaks. One `<hr>` only, or use a `<div>` with top+bottom borders for the visual effect.**
- **Uppercase hex colors → TinyMCE lowercases on save. Write `#1b5e20` from the start so source matches stored copy.**
